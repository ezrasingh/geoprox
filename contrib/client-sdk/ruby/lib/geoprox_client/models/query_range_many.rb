  ...
  class QueryRangeMany
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `GeoproxClient::QueryRangeMany` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `GeoproxClient::QueryRangeMany`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @indices.nil?
        invalid_properties.push('invalid value for "indices", indices cannot be nil.')
      end
      ...
      if @range > 65535
        invalid_properties.push('invalid value for "range", must be smaller than or equal to 65535.')
      end
    ...
    def range=(range)
      ...
      if range > 65535
        fail ArgumentError, 'invalid value for "range", must be smaller than or equal to 65535.'
      end
    ...
    def hash
      [count, indices, lat, lng, range, sorted].hash
    end
