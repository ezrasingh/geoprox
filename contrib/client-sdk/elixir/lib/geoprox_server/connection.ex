...
defmodule GeoproxServer.Connection do
  @moduledoc """
  Handle Tesla connections for GeoproxServer.

  Additional middleware can be set in the compile-time or runtime configuration:

      config :tesla, GeoproxServer.Connection,
        base_url: "http://localhost",
        adapter: Tesla.Adapter.Hackney

  The default base URL can also be set as:

      config :geoprox_client,
        :base_url, "http://localhost"
  """
  ...
  def middleware(options \\ []) do
    base_url =
      Keyword.get(
        options,
        :base_url,
        Application.get_env(:geoprox_client, :base_url, @default_base_url)
      )
    ...
    user_agent =
      Keyword.get(
        ...
        Keyword.get(
          tesla_options,
          :user_agent,
          "openapi-generator - GeoproxServer 0.5.0 - elixir"
        )
      )
