/*
 * geoprox-server
 *
 * Geoprox server implementation providing a HTTP API for geospatial queries and position tracking
 *
 * The version of the OpenAPI document: 0.5.0
 * Contact: singhezra@gmail.com
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */


using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
using System.IO;
using System.Runtime.Serialization;
using System.Text;
using System.Text.RegularExpressions;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using Newtonsoft.Json.Linq;
using System.ComponentModel.DataAnnotations;
using OpenAPIDateConverter = GeoproxClient.Client.OpenAPIDateConverter;

namespace GeoproxClient.Model
{
    /// <summary>
    /// Arguments for inserting a key
    /// </summary>
    [DataContract(Name = "InsertKey")]
    public partial class InsertKey : IValidatableObject
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="InsertKey" /> class.
        /// </summary>
        [JsonConstructorAttribute]
        protected InsertKey() { }
        /// <summary>
        /// Initializes a new instance of the <see cref="InsertKey" /> class.
        /// </summary>
        /// <param name="key">Object key (required).</param>
        /// <param name="lat">Latitude (required).</param>
        /// <param name="lng">Longitude (required).</param>
        /// <param name="ttl">The time-to-live (TTL) for this key, in seconds.</param>
        public InsertKey(string key = default(string), double lat = default(double), double lng = default(double), long? ttl = default(long?))
        {
            // to ensure "key" is required (not null)
            if (key == null)
            {
                throw new ArgumentNullException("key is a required property for InsertKey and cannot be null");
            }
            this.Key = key;
            this.Lat = lat;
            this.Lng = lng;
            this.Ttl = ttl;
        }

        /// <summary>
        /// Object key
        /// </summary>
        /// <value>Object key</value>
        [DataMember(Name = "key", IsRequired = true, EmitDefaultValue = true)]
        public string Key { get; set; }

        /// <summary>
        /// Latitude
        /// </summary>
        /// <value>Latitude</value>
        [DataMember(Name = "lat", IsRequired = true, EmitDefaultValue = true)]
        public double Lat { get; set; }

        /// <summary>
        /// Longitude
        /// </summary>
        /// <value>Longitude</value>
        [DataMember(Name = "lng", IsRequired = true, EmitDefaultValue = true)]
        public double Lng { get; set; }

        /// <summary>
        /// The time-to-live (TTL) for this key, in seconds
        /// </summary>
        /// <value>The time-to-live (TTL) for this key, in seconds</value>
        [DataMember(Name = "ttl", EmitDefaultValue = true)]
        public long? Ttl { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("class InsertKey {\n");
            sb.Append("  Key: ").Append(Key).Append("\n");
            sb.Append("  Lat: ").Append(Lat).Append("\n");
            sb.Append("  Lng: ").Append(Lng).Append("\n");
            sb.Append("  Ttl: ").Append(Ttl).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public virtual string ToJson()
        {
            return Newtonsoft.Json.JsonConvert.SerializeObject(this, Newtonsoft.Json.Formatting.Indented);
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        IEnumerable<ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
        {
            // Ttl (long?) minimum
            if (this.Ttl < (long?)0)
            {
                yield return new ValidationResult("Invalid value for Ttl, must be a value greater than or equal to 0.", new [] { "Ttl" });
            }

            yield break;
        }
    }

}
